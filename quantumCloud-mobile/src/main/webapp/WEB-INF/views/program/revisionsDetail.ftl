	...
	<div class="weui-cells" id="${qcodeTask.id}">
	    ...
	    <div class="case-list">
	    	<#if (qcodeTask.status ==  QcodeTask.STATUS_WAITING) || (qcodeTask.status ==  QcodeTask.STATUS_PROCESS)>
	        	<a href="###" class="blue" onclick="App.confirm({message:'确定终止该任务？',callback:'terminateTask',params:{id:${qcodeTask.id}}});">终止</a>
	         	<a href="###" class="red"  onclick="App.confirm({message:'确定删除该任务？',callback:'delTask',params:{id:${qcodeTask.id}}});">删除</a>
	        <#else>
		        <a href="###" class="red" style="width: 98%;" onclick="App.confirm({message:'确定删除该任务？',callback:'delTask',params:{id:${qcodeTask.id}}});" >删除</a>
	        </#if>
	    </div>
	</div>
  </#list>
			...
			for(var i=0;i<dataObj.length;i++){
				...
				if(obj!=null){
					...
					if((obj.status == STATUS_WAITING) || (obj.status == STATUS_PROCESS)){
						content += '	<a href="###" class="blue" onclick="App.confirm({message:'+"'确定终止该任务？'"+',callback:'+"'terminateTask'"+',params:{id:'+obj.id+'}});">终止</a>';
						content += '	<a href="###" class="red"  onclick="App.confirm({message:'+"'确定删除该任务？'"+',callback:'+"'delTask'"+',params:{id:'+obj.id+'}});">删除</a>'; 
					}else{
						content += '	<a href="###" class="red" style="width: 98%;" onclick="App.confirm({message:'+"'确定删除该任务？'"+',callback:'+"'delTask'"+',params:{id:'+obj.id+'}});">删除</a>';
					}
					content += '	</div>'; 
					content += '</div>'; 
				}
  ...
  $.post(url, param, function(response) {
    ...
    if (response.success) {
        ...
        if (status == 3 || status == 5) {
            ...
            App.prompt(messages.task_status_syntactic_error);
            setTimeout(function() {window.location.reload()}, 1500);
        }
    } else {
