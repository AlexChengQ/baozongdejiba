...
<#escape x as x?html> 
	...
	<ol class="breadcrumb">
		<li></li>
       <li><a href="${base }/blockConfig/index.html">版块管理</a></li>
	...
	<div class="row">
		...
		<@main.warning/>
		 <div class="form-group">
			<label for="title" class="col-md-3 control-label no-padding-right"> 名称 </label>
			<div class="col-md-6">
         ...
         <div class="form-group">
            ...
            <div class="form-group file">
                <label for="pic" class="col-md-3 control-label no-padding-right"> 版块图片 </label>
                <div class="col-md-6">
            ...
            <div class="form-group" id="isHide" hidden>
...
<script type="text/javascript">
	...
	jQuery(function($) {
		...
		$(".btn-save").click(function(){
		    var count = getBlockCount($("#name").val());
		    if(count > 0) {
                var n = noty({
                    text        : "已添加相同名字版块",
                    type        : 'error',
			...
			if($("#myFormId").validateForm(saveBlockConfig)) {
				$.ajax({  
			        type:'post',
                    traditional :true,
                    url:'${base}/blockConfig/save.json',
	...
	function getBlockCount(name) {
        var flag = 0;
        $.ajaxSettings.async = false;
        $.post("${base}/blockConfig/getHaveBlockName.json",{"name":name},function(result) {
            if(result) {
                flag = result;
            }
        })
