...
<@main.body>
    ...
    <section class="content">
        ...
        <div class="row">
            <div class="col-md-12">
                <form method="post" id="searchForm" action="${base}/Qcode/dic/getDicContentById.html?dicId=${(dicId)!}" autocomplete="off">
                        ...
                        <div class="box-header with-border">
                            <div class="btn-group">
                                <#--<a class="btn btn-xs btn-primary" onclick="return onAction({url:'${base}/SysRole/add.html',title:'新增',callback:refreshList})">新增</a>-->
                                    <a class="btn btn-sm btn-primary" href="${base}/Qcode/dic/saveContent.html?dicId=${dicId}">新增</a>
                            </div>
                        </div>
                                ...
                                <tbody>
                                    <#if (pager.entityList)?exists && ((pager.entityList)?size != 0)>
                                        ...
                                        <tr>
                                            ...
                                            <td title="${(entity.dicdesc)!}">${(entity.diccontent)!}</td>
                                            <td title="${(entity.dicname)!}">${(entity.dicflag)!}</td>
                                            ...
                                            <td>
                                                ...
                                                <#--.id)!}">-->
                                                    <#--详情-->
                                                <#--</a>-->
                                                <a href="javaScript:;" onclick="delDicContent('${entity.id}')">删除</a>
                                            </td>
                                        </tr>
                                        </#list>
                                    <#else>
...
<script type="text/javascript">
    ...
    //删除字典
    function delDicContent(id) {
        alert(111)
        Dialog.confirm({message: "确定删除吗？"}).on(function (callback) {
            if (callback) {
                $.ajax({
                    type: 'post',
                    url: "${base}/Qcode/dic/delDicContent.json",
                    data: {
                        "id":id
                    },
                    dataType: 'json',
