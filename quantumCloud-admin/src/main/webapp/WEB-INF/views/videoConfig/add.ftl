...
<@main.body>
    ...
    <section class="content">
        <ol class="breadcrumb">
            <li></li>
            <li><a href="${base }/videoConfig/index.html">视频管理</a></li>
                    ...
                    <div class="box box-primary">
                        <div class="box-body">
                            ...
                            <div class="form-group" hidden id="hideUrl">
                                ...
                                <div class="col-md-6">
                                    <input type="text" id="video_url" name="video_url" value="http://video.qpanda.cn:65520/" class="form-control"/>
                                </div>
...
<script type="text/javascript">
    ...
    uploader.on('uploadSuccess', function(file) {
        ...
        $.post("${base}/videoConfig/uploadSuccess.json", { "guid": guid,chunks:chunks,fileName:file.name,catalog:$("#catalog").val(),videoType:$upload.attr("data-value"),vid:$("#vid").val()},
                function(data){
                    var path = data.obj;
    ...
    uploader1.on('uploadSuccess', function(file) {
        ...
        var chunks = Math.ceil(file.size/(100*1024*1024));
        $.post("${base}/videoConfig/uploadSuccess.json", { "guid": guid,chunks:chunks,fileName:file.name,catalog:$("#catalog").val(),videoType:$upload1.attr("data-value"),vid:$("#vid").val()},
                function(data){
                    var path = data.obj;
    ...
    jQuery(function($) {
        ...
        $(".btn-save").click(function(){
            var count = getHaveVideo($("#title").val());
            if(count > 0) {
                var n = noty({
                    text        : "已添加相同标题视频",
                    type        : 'error',
    ...
    function getCatalog() {
        if(changeCatalogDel == 1) {
            $.ajaxSettings.async = false;
            $.post("${base}/videoConfig/delUploadVideo.json",{"vid":$("#vid").val()},function (result) {
            })
            $.ajaxSettings.async = true;
        }

        $("#catalog").val( $("#block").find("option:selected").attr("data-value"));
        $.post("${base}/videoConfig/getVid.json",{"block":$("#block").val()},function (result) {
            $("#vid").val(result);
        })
    ...
    function getHaveVideo(title) {
        var flag = 0;
        $.ajaxSettings.async = false;
        $.post("${base}/videoConfig/getHaveVideo.json",{"title":title},function(result) {
            if(result) {
                flag = result;
            }
        })
