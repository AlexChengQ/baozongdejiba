package com.bylz.quantumCloud.utils;
...
public abstract class DateUtils {
	...
	public static void  main(String[] args){
	    //System.out.println(getYear() + " month:" + (month()+1) + " " + dayOfMonth());
	    //System.out.println("---" + getLastDayOfMonth(getYear(),(month()+1)));
	    System.out.println(getMonthDays(-6));
	 ...
	 * @return
	 */
	public static Map<String,Object> getMonthDays(int month){
		Map<String, Object> daysMap = new HashMap<String, Object>();
		Date date = getDateWithMonth(month);
	    Calendar c = calendar();
		c.setTime(date);
		
		List<String> days = new ArrayList<String>();
		
		int lastDay = getLastDayOfMonth(c.get(Calendar.YEAR), (c.get(Calendar.MONTH) + 1));
		for(int i = 1; i <= lastDay; i ++){
			days.add(i + "");
		}
		
		daysMap.put("year", c.get(Calendar.YEAR));
		daysMap.put("month", (c.get(Calendar.MONTH) + 1));
	 ...
	 * 根据日期 计算时间加减
	 * @param date 传入的时间
	 * @param minute 需要加或减的时间（以分钟为单位） 如果是计算分钟减 传入负数 如-30（日期减去30分钟）
	 * @author zhanglang
	 * @return
	 */
	public static Date addMiunte(Date date, int minute){
		Calendar calenar = calendar();
		calenar.setTime(date);
		calenar.add(Calendar.MINUTE, minute);
		return calenar.getTime();
	}
	
	/**
	 * 将字符串日期时间转换成java.util.Date类型(解决并发转换报错的问题)
	 * <p>
	 * 日期时间格式yyyy-MM-dd HH:mm:ss
	 * @param datetime
	 * @return
	 */
	public static Date parseDatetimeWithNewFormat(String datetime) {
		if (datetime == null || datetime.length() == 0) {
			return null;
		}
		try {
			SimpleDateFormat dtFormat = new SimpleDateFormat(DateStyle.YYYY_MM_DD_HH_MM_SS.getValue());
