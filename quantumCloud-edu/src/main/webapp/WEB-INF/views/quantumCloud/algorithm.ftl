...
//                console.log($(this).find("a").attr("data-value"))
         console.log(mainTopArr)
         $('html, body').animate({scrollTop:mainTopArr[($(this).find("a").attr("data-value"))]-60}, 400);
 ...
 $(".algorithm_con ul li").on("click",function () {
     $(".news-box").hide();
     $(this).addClass("active").siblings().removeClass("active");
     $(".algCollCode").eq($(this).index()).addClass("active").siblings().removeClass("active");
     $("#nlName").html($(".algorithm_con ul li.active a").text())
     $(".alt_n_code").getNiceScroll().resize();
     $(".alt_n_code").getNiceScroll().show();
     getSide()
 })
var hashNum= parseInt(window.location.hash.substring(1,1));
 if(hashNum>0&&hashNum<7){
     $(".algorithm_con ul li").eq(hashNum).click()
 }
