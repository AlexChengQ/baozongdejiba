...
<div class="box">
    <div class="box-left">
        ...
        <div class="ls">
    			<h2><@spring.message code="version_logs" /></h2>
        <#list versionMapList as versionMap>            
        <div class="jg">
            <div class="implement-one">
                <div class="xm-box">
                <div class="f16 l xm"><#if lang?? && lang?string=="en_US">${qcodeProject.nameEnglish!''}<#else>${(qcodeProject.name)!}</#if>#${(versionMap.version).createTime?string('yyyyMMddHHmm')} ${(versionMap.version).name!}</div>
                <div class="xm-but">
                ...
                <div class="xm-box-list">
                    <div class="l" style="padding: 16px;" onclick="getVersion(${(versionMap.version).id!});">
    						...
    						<#if task.status??&&task.status=='1'>
		                        <div class="new-box">
		                            <div class="new-title">
		                                ...
		                                <span class="zt-but"><a onclick="terminateTask(${task.id!});" style="cursor:pointer"><@spring.message code="task_status_termination" /></a></span>
		                                <span class="zt-del"><a onclick="delTask(${task.id!});" style="cursor:pointer"><@spring.message code="task_status_delete" /></a></span>
		                            </div>
		                        </div>    						
                        	<#--处理中-->
    						...
    						<#elseif task.status??&&task.status=='3'>
			                        <div class="new-box">
			                            <div class="new-title">
			                                ...
			                                ${task.startTime?string("yyyy-MM-dd HH:mm")}
			                                <span class="zt-but"><a onclick="showData(${task.id!});" style="cursor:pointer"><@spring.message code="task_status_result" /></a></span>
			                                <span class="zt-del"><a onclick="delTask(${task.id!});" style="cursor:pointer"><@spring.message code="task_status_delete" /></a></span>
			                            </div>
			                        </div>    						
    						<#--语法错误-->
    ...
    $.ajax({
        ...
        success:function(result){
            ...
            if(result.success){
                ...
                <#if lang?? && lang?string=="en_US">
                    qcodeProjectName = "${qcodeProject.nameEnglish!}";
                <#else>
                    qcodeProjectName = "${(qcodeProject.name)!}";
                </#if>
